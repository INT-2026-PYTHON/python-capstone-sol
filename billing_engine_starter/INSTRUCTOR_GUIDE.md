...
### What they implement
- The reduced Day 2 repository spine: Customer, Plan, PlanTier, Discount, Subscription read/due methods, Usage, Invoice add/get, and InvoiceLineItem.
- `billing/pipeline.py::build_invoice`.
- They read `billing/cycle.py::BillingCycle.run` and identify the repository helpers they will finish on Day 3.

Leave these for later:
- Day 3: `SubscriptionRepository.update_status`, `SubscriptionRepository.update_period`, `InvoiceRepository.count_for_subscription`, `LedgerRepository.add`, `LedgerRepository.list_for_customer`.
- Day 4: `InvoiceRepository.mark_paid`, `InvoiceRepository.mark_failed`, `InvoiceRepository.set_pdf_path`.
- Payment groundwork: `PaymentAttemptRepository` unless the cohort is clearly ahead.

### Debugging Day 2 (watch for these)

**"My test says the row is None but I know I inserted it"**
- Check: Are they using separate `connect()` calls for INSERT and SELECT? SQLite's default isolation can hide uncommitted writes.
- Solution: INSERT must be inside a transaction (`with db.transaction()`), not just `db.connect()`.
- Show them: `conn.commit()` happens automatically when exiting the `transaction()` context manager.

**"AttributeError: 'NoneType' object has no attribute 'id'"**
- This means they forgot to populate the returned object after an INSERT.
- In `add()` methods, after `INSERT`, call `cur.lastrowid` and return a new instance with `id=new_id`.
- Show them the pattern:
  ```python
  cur = conn.execute("INSERT INTO customers ... VALUES (?, ?, ...)", (...))
  new_id = cur.lastrowid
  return Customer(id=new_id, name=..., email=...)
  ```

**"TypeError: unsupported operand type(s) for -: 'Money' and 'NoneType'"**
- They probably returned `None` from a repository query instead of raising or defaulting.
- Check their `if row is None: return None` logic. Confirm they always return an object or None, never forget a return statement.

**"ValueError: Cannot mix currencies"**
- Usually happens in `build_invoice` when discount or tax doesn't match the strategy's currency.
- Trace: Is the plan's currency being threaded through? Are all Discounts/Taxes created with the same currency?
- Show them: `discount.apply()` returns a `Money` — confirm it has the right currency.

**"sqlite3.IntegrityError: FOREIGN KEY constraint failed"**
- They tried to insert a subscription for a nonexistent customer or plan.
- Show them: Always insert parent rows first (customers, plans) before child rows (subscriptions).
- In tests, check the `_setup` fixture is being called.

**"UNIQUE constraint failed: invoices.subscription_id, invoices.period_start"**
- This constraint is intentional. Day 2 students should understand what it protects, but Day 3 is where they handle it in the workflow.
- In Day 2 repository tests, this should only appear in the duplicate-invoice test.

**"Cannot determine the currency"**
- When reading back Money from the DB, they forgot to pass the currency.
- `Money(row["amount"], currency)` — they must include the currency parameter!
- Common mistake: `Money(row["subtotal"], "INR")` hardcoded instead of `Money(row["subtotal"], row["currency"])`.

**"OperationalError: table invoices has no column named 'line_items'"**
- They tried to SELECT a field that doesn't exist in the schema.
- Remind: `line_items` are stored in a separate table (`invoice_line_items`), not in `invoices`.
- To get an invoice with line items, they must query both tables.

### Check-ins
- **10:00** — `CustomerRepository` green? They need this to debug anything else.
- **12:30** — "How many reduced-scope repositories are green? At least 5 expected before lunch (Customer, Plan, PlanTier, Discount, Subscription)."
- **15:00** — `test_pipeline.py` — all green? If not, walk through ONE failing case with them. Check discount and tax application.
- **17:00** — Ask them to:
  1. Explain the UNIQUE constraint on invoices and why it matters.
  2. Predict what `BillingCycle.run` will do on Day 3 when given a due subscription.
  3. Verify `build_invoice` works in a REPL.

### Common Day-2 mistakes to watch for
- Forgetting `currency` when reading back Money from DB rows — amounts are TEXT without currency metadata.
- Catching `sqlite3.IntegrityError` too broadly — they should understand WHICH constraint failed (FK vs UNIQUE).
- Trying to implement ledger/payment methods too early — keep Day 2 focused on persistence basics and pipeline.
- Returning hardcoded currencies instead of reading `row["currency"]` — this breaks multi-currency systems.
- Mixing up `db.connect()` (for reads) and `db.transaction()` (for writes). Explain the difference clearly.

### Drop-order if behind
1. Leave all Day 3/4 repository stubs untouched.
2. Skip PlanTier/Discount polish before skipping Customer/Plan/Subscription/Invoice basics.
...
### What they implement
- Repository helpers intentionally deferred from Day 2: subscription status/period updates, invoice count, and ledger add/list.
